...
export default function UnitMapPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/content/${encodeURIComponent(unitId)}`);
        if (!res.ok) {
          if (!cancelled) setState({ kind: 'error', message: 'Unit is on the way.' });
          return;
        }
        ...
        if (!cancelled) setState({ kind: 'error', message: 'Could not load unit.' });
      }
    })();
    return () => {
      cancelled = true;
  ...
  return (
    <main className="flex min-h-dvh flex-col bg-[var(--color-surface)]">
      <TopBar
        title={state.kind === 'ready' ? state.unit.title : 'Unit'}
...
function LessonCard({ lesson, stars, onSelect }: LessonCardProps) {
  return (
    <button
      type="button"
      onClick={onSelect}
      aria-label={`${lesson.title}, ${stars} of 3 stars`}
